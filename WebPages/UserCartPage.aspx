...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <div class="col-10 container" style="font-family: 'Comfortaa', cursive">
             ...
             <asp:Repeater ID="CartItmRptr" OnItemCommand="CartItmRptr_ItemCommand" runat="server">
                 <ItemTemplate>
                             ...
                             <div class="row d-flex border rounded-4 mt-4  flex-md-row mb-4 shadow h-25 w-75 " style="display:flex">

                                 <div class="col-auto d-none d-lg-block mx-3 my-3">
          <img class="bd-placeholder-img" style="width:15vw" src="../Images/ProductImages/<%#Eval("ProductId") %>/<%# Eval("ImageName")%><%# Eval("ImageExtention") %>" alt="<%#Eval("ImageName") %>"/>          

        </div>
        <div class="col p-4 d-flex flex-column position-static">
          <strong class="d-inline-block mb-2 text-primary"><%#Eval("BrandName") %></strong>
          <p class="my-2"><%#Eval("ProductName") %></p>
            
          <p class="my-2 text-danger" style="text-decoration:line-through;color:red">₹ <%#Eval("ProductPrice") %></p>
            
                      <p class="mb-0 " > ₹ <%#Eval("ProductSellingPrice","{0:000,0}") %></p>

          <br />
            <div class="d-flex justify-content-evenly">


             <a href="PaymentPage.aspx?PID=<%#Eval("ProductId")%>&BuyAll=false" class="rounded-4 text-light p-2 text-center bg-primary">Buy Now</a>
             <asp:Button ID="RemoveItemBtn" runat="server" CssClass="rounded-4 text-light border border-0 p-2 text-center bg-danger" OnClick="RemoveItem_Click" CommandArgument='<%# Eval("ProductId") %>' Text="Delete" UseSubmitBehavior="False" />
    ...
    <div class="col-3 h-50 position-fixed top-50 end-0 translate-middle-y sticky-top" id="summerybox" runat="server" style="font-family: 'Comfortaa', cursive;z-index:5000">
